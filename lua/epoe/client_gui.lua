...
local function make_url(url)
	if epoe_parse_steamids:GetBool() then
		if url:find'^76561[0123]%d%d%d%d+$' then return 'http://steamcommunity.com/profiles/' .. url end
...
local function AppendTextLink(a, callback)
	...
	local checkpatterns = {'https?://[^%s%\']+', 'ftp://[^%s%\']+', 'steam://[^%s%\']+'}
...
function PANEL:Init()
	...
	Cfg.OnMousePressed = function(_, ...)
		self.OnMousePressed(self, ...)
	end

	Cfg.OnMouseReleased = function(_, ...)
	...
	Button:SizeToContents() -- gah
	Button:SetDrawBorder(false)
	Button:SetTall(16)
	Button:SetWide(Button:GetWide() + 6)
	Cfg:AddPanel(Button)
	local Button = vgui.Create('DButton', self)
...
function PANEL:AppendTextX(txt)
	...
	if lmode == 0 then return self:AppendText(txt) end
	...
	local function func(link, url, real_url)
		...
		if link then
			self.RichText:AddLink(function()
				self:ResetLastColor()
				self:AppendText(url)
			end, function()
				local lmode = epoe_links_mode:GetInt()
...
function PANEL:Think()
	...
	if mx > px and mx < px + self:GetWide() and my > py and my < py + self:GetTall() then
		self.being_hovered = true
	...
	if remainvisible:GetBool() and hook.Call('HUDShouldDraw', GAMEMODE, 'CHud' .. TagHuman) == false and not self.being_hovered then
		self:SetAlpha(0)
	...
	if self.Hovered and mx > (self.x + self:GetWide() - 20) and my > (self.y + self:GetTall() - 20) then
		--self.m_bSizable and
		self:SetCursor('sizenwse')
...
function PANEL:OnMousePressed(mc)
	if mc == MOUSE_RIGHT or (gui.MouseX() > (self.x + self:GetWide() - 20) and gui.MouseY() > (self.y + self:GetTall() - 20)) then
...
local function epoe_toggle(_, cmd, args)
	if cmd == '+epoe' then
		...
		if IsValid(egui) then
			...
			-- Doubleclick
			if lastclick + threshold > RealTime() then
				lastclick = 0 -- reset
				e.GUI:ToggleActive()
			else
				lastclick = RealTime()
